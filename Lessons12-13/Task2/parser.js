...
function printGeneratedNumbers(event) {
    clearCanvas();
    generateRandomNumbers(event.currentTarget.max, event.currentTarget.len).forEach(element => printCanvas(element));
    taskFunc(event.currentTarget.numbers)
}

function taskFunc(x) {
    // console.log(findLenBtwDots(x[0], x[1]))
...
function main() {
    ...
    genButton.numbers = generateRandomNumbers(500, 100);
    genButton.max = 500;
    genButton.len = 100;
